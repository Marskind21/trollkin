# Trollkin Extension notes

## Categories:

### EnderMars21:
  - EnderMars21

### NeunEinser:
  - NeunEinser
  - Evil NeunEinser

### Lingo:
  - [lingo] Lingo Smily
  - [upsidedown_lingo] Upside Down Lingo Smily
  - no_lingo_clover *

### [mego] Toys:
  - [mego] Mego Brick (3)
  - [drawing] Master Piece (2)
  - Bouncy Ball (1)

### no_leaf_clover:
  - no_leaf_clover
  - no_lingo_clover *

### [pixel] Computer Stuff:
  - Pixel (4)
  - Tablet (2)
  - Code (1)

### slicedlime:
  - slicedlime (5)
  - [lime_luck] limeluck (1)

### Charcoal:
  - Charcoal (1)
  - [charcoal_block] Block of Charcoal (2)

### Anvil:
  - Anvil (1)
  - Chipped Anvil (2)

### Netherrack:
  - Netherrack (1)
  - Nether Brick (2)
  - Nether Bricks (2)
  - Cracked Nether Bricks (3)
  - Chiseled Nether Bricks (2)

### [sus] Sus Blocks:
  - Suspicious Sand
  - Suspicious Gravel

### [common] Common Items:
  - Wooden Pickaxe (3)
  - Stone Pickaxe (2)
  - Stone Axe (1)
  - Stone Shovel (1)
  - Crafting Table (4)

### Slow Copper:
  - Exposed Copper (1)
  - Exposed Copper Grate (1)
  - Exposed Copper Bars (1)
  - Exposed Copper Chain (1)
  - Exposed Copper Golem Statue (4)

### Sad Obsidian:
  - Crying Obsidian
  - Respawn Anchor *

### [annoying] Just Annoying:
  - Rabbit Stew (3)
  - Crafter (2) *
  - Light Gray Stained Glass Pane (1)
  - Beetroot Soup (3)

### Rainbow:
  - Rainbow Dye (1)
  - Rainbow Glass Pane (3)
  - Rainbow Wool (2)

### Pufferfish:
  - Pufferfish (1)
  - Puffy (4)

### Brian:
  - Brian (1)
  - Charged Brian (3)
  - Super Charged Brian (6)

### SpielmitStil:
  - SpielmitStil

### Flowers:
  - Flower Bundle (10)
  - Cornflower
  - Dandelion
  - Open Eyeblossom (2)
  - Oxeye Daisy
  - Poppy
  - Blue Orchid (2)
  - Pink Petals
  - Wildflowers

### Chop Chop:
  - Log Collection
  - Plank Collection
  - Leaf Collection
  - Sapling Collection (3)

### Butcher:
  - Raw Beef
  - Raw Chicken
  - Raw Mutton
  - Raw Porkchop
  - Raw Rabbit
  - Raw Monstrosity (5)

### [random] Funny Random Stuff:
  - Glow Berry Bush
  - TNT Yeeter
  - [parry_the_otter] Parry the otter and the game of fetching
  - Coins

### Scraping Tool™:
  - Scraping Tool™
  - Pumkin Hide (use scraping tool on pumpkin)
  - Melon Hide (use scraping tool on melon)
  - Piston Head (use scraping tool on piston)
  - Granite *

### [stone] Stone-ite:
  - Granite *
  - Diorite
  - Andesite (2)

### Blackstone:
  - Blackstone
  - Gilded Blackstone (3)
  - Cracked Polished Blackstone Bricks
  - Chiseled Polished Blackstone

### Fast Copper:
  - Copper Chestplate
  - Copper Axe
  - Copper Hoe
  - Copper Lantern

### END:
  - Chorus Fruit
  - Purpur Stairs
  - Purpur Pillar
  - End Stone
  - End Stone Brick Wall

### [glowstone] Glowing Stones:
  - Glowstone
  - Redstone Lamp *
  - Respawn Anchor (2) *

### Quartz:
  - Quartz
  - Chiseled Quartz Block
  - Observer (3) *
  - Daylight Detector *

### Redstone:
  - Redstone Lamp (2) *
  - Observer (3) *
  - Daylight Detector *
  - Crafter (2) *
  - Note Block
  - Redstone Torch

### [tuff] Tough Tuff:
  - Tuff
  - Chiseled Tuff (2)
  - Chiseled Tuff Bricks (2)

### Netherite:
  - Netherite Scrap
  - Netherite Ingot (2)
  - Netherite Block (3)

### [gambling] Let's go gambling!:
  - Chip
  - Slot Machine
  - Lucky Wheel

### Hollow Knight:
  - Knight
  - Hornet

### [shelf] Shelves:
  - Bookshelf (4)
  - Chiseled Bookshelf (1)
  - Pale Oak Shelf (2)
  - Bamboo Shelf (2)

### Nether Star:
  - Nether star (1)
  - Beacon (3)

### [disc] Music Disc:
  - Music Disc (Trollkin V3 Soundtrack)
  - [music_disc_slow] Music Disc (Trollkin V3 Soundtrack (Slow))
  - [music_disc_fast] Music Disc (Trollkin V3 Soundtrack (Fast))

### Loading Bar:
  - Loading Bar 0/3 (1)
  - Loading Bar 1/3 (2)
  - Loading Bar 2/3 (3)
  - Loading Bar 3/3 (4)

### Blaze:
  - Blaze Rod
  - Brewing Stand (2)
  - Copper Bulb (3)
  - Fire Charge (2)

### Shulker:
  - Shulker Shell
  - Shulker Box (2)

### Breeze:
  - Breeze Rod
  - Wind Charge

### Soul:
  - Skeleton Skull
  - Soul Sand (2)
  - Soul Soil (2)
  - Soul Torch (3)
  - Soul Lantern (5)
  - Soul Campfire (10)

### Diamond Ore:
  - Glow Lichen
  - Diamond Ore (7)
  - Deepslate Diamond Ore (7)

### Ghast:
  - Ghast Tear
  - Dried Ghast (8)
  - Music Disc (Tears) (3 !)

### Mob Discs:
  - Music Disc (Tears) (!)
  - Music Disc (Lava Chicken)

### Player Armor (Armadillo scutes):
  - Player Helmet (Emerald Trim)
  - Player Chestplate (Diamond Trim)
  - Player Leggings (Resin Trim)
  - Player Boots (Amthyst Trim)

### Trollkin:
  - Trollkin (Carved Pumpkin) (2)
  - Lit Trollkin (Jack o Lantern)

## New Items (not in the Pool):
  - Interdimensional Teleporter (2 crying obsidian + 4 glowstone + ender pearl + 2 diamonds)
	- Placed on Chiseled Tuff: Spawns Breezes
	- Placed on Magma Block: Spawns Magma Cubes
	- Placed on Soul Soil: Spawns Ghasts and Skeletons
	- Placed on Netherrack: Spawns Zombified Piglins
	- Placed on Netherbricks: Spawns Wither Skeletons and Blazes
	- Placed on Endstone: Spawns Endermen
	- Placed on Purpur Block: Spawns Shulkers

## Changes:
  - Coppergolems turn into statues really fast.
  - Each Zombie/Husk/Drowned has an 50% chance to become a baby and 50% of babies are chicken jockeys.
  - Randomly choose between slow/normal/fast playing speed when starting the music.
  - Players and most mobs rescale according to their health.

## Advancements:
  - no_leaf_clover's Tool | Craft a Scraping Tool™.
    - Give me that Slime! | Use the Scraping Tool™ on a sticky piston.
    - Why? Just why? | Use a Scraping Tool™ on a pumpkin or melon.
    - Decapitation | Decapitate a piston.
    - Below the Flesh | Use a Scraping Tool™ on netherrack.
      - Harder than Steel | Obtain a netherite scrap from scraping netherrack.
    - Mineral Extraction | Use a Scraping Tool™ on diorite or ganite.
  - End Update! | Craft an item that is usually only obtainable in the End.
  - Glow and Behold! Stone Age! | Obtain Glowstone.
  - The Creator of Fetchr | Obtain NeunEinser.
  - The Creator of this madness | Obtain EnderMars21.
  - I could read that forever. | Obtain the "Parry the otter" novel. (Sorry, but you can't actually read it.)
  - Big Pearl | Craft an Interdimensional Teleporter
    - Very Windy | Use an Interdimensional Teleporter on chiseled tuff.
	- Jumping Magma?! | Use an Interdimensional Teleporter on a magma block.
	- Spooky Ghosts | Use an Interdimensional Teleporter on soul soil.
	- Undead Pigs | Use an Interdimensional Teleporter on netherrack.
	- A Terrible Fortress at Home | Use an Interdimensional Teleporter on nether bricks.
	- Ender Pearl Farm? | Use an Interdimensional Teleporter on end stone.
	- Not the City at the End of the Game | Use an Interdimensional Teleporter on a purpur block.
